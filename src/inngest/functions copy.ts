...
//             console.log(`[create-sandbox] ${msg}`);
//           });
//         },
//       });
...
//           }
//         }
//         if (network.state.data.summary) return undefined;
//         return agent;
//       },
//     });

//     // -------------------- STEP 5: Split high-level prompt into TODO list --------------------
//     const todoSplitter = createAgent({
//       name: "todo-splitter",
//       description: "Breaks high-level prompts into TODOs",
//       system: `You're a task planner. Split the user input into a list of step-by-step technical TODOs for a software engineer to follow.`,
//       model: openai({ model: "gpt-4o", defaultParameters: { temperature: 0.2 } }),
//     });

//     const { output: todoListOutput } = await throttleRun("split-todos", () =>
//       todoSplitter.run(event.data.value), {
//         logger: async (msg) => {
//           await step.run("log", async () => {
//             console.log(`[TODO Splitter] ${msg}`);
//           });
//         }
//     });

//     const todos: string[] = Array.isArray(todoListOutput)
//       ? todoListOutput
...
//     await step.run("log", async () => {
//       console.log(`✅ TODO list extracted (${todos.length} items).`);
//     });

//     // -------------------- STEP 6: Run each TODO sequentially --------------------
//     for (let i = 0; i < todos.length; i++) {
//       const todo = todos[i];
//       await step.run("log", async () => {
//         console.log(`🧩 Processing TODO [${i + 1}/${todos.length}]: ${todo}`);
//       });

//       await throttleRun(`todo-${i + 1}`, () =>
//         network.run(todo, { state }), {
//         logger: async (msg) => {
//           await step.run("log", async () => {
//             console.log(`🧠 [TODO ${i + 1}] ${msg}`);
//           });
//         },
//       });

//       await step.run("log", async () => {
//         console.log(`✅ Done with TODO [${i + 1}/${todos.length}]: ${todo}`);
//       });
//     }

//     // After loop: wrap final result
//     const runResult = {
//       state: network.state,
//       _history: (network as any)._history ?? [],
//     };

//     // -------------------- STEP 7: Fallback summary handling --------------------
//     let summary = runResult.state.data.summary;

//     if (!summary || typeof summary !== "string") {
//       // @ts-ignore - accessing internal _history for fallback
//       const history = runResult._history as any[] | undefined;
//       const lastAssistant = history
//         ?.reverse()
//         ?.find((entry) => entry?.output && Array.isArray(entry.output))
...
//     // Determine if it's an error (e.g. no files or no summary)
//     const isError =
//       !runResult.state.data.summary ||
//       Object.keys(runResult.state.data.files || {}).length === 0;

//     // -------------------- STEP 8: Generate title and assistant response --------------------
//     const fragmentTitleGenerator = createAgent({
//       name: "fragment-title-generator",
//       description: "Generate a short, clear title for the result fragment",
//       system: FRAGMENT_TITLE_PROMPT,
...
//       description: "Generate a final user-friendly response",
//       system: RESPONSE_PROMPT,
//       model: openai({ model: "gpt-4o", defaultParameters: { temperature: 0.1 } }),
//     });
...
//     // -------------------- STEP 9: Obtain public sandbox URL --------------------
//     const sandboxUrl = await step.run("get-sandbox-url", async () => {
//       const sb = await throttleRun("get-sandbox", () => getSandbox(sandboxId));
//       return `https://${sb.getHost(3000)}`;
//     });

//     // -------------------- STEP 10: Persist result to database --------------------
//     await step.run("save-result", async () => {
//       if (isError) {
//         return await prisma.message.create({
//           data: {
...
//                 sandboxUrl,
//                 title: generateFragmentTitle(),
//                 files: {}, // or leave undefined if Prisma allows
//               },
//             },
//           },
//         });
