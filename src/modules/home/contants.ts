export const PROJECT_TEMPLATES = [
    ...
    `},
   {
    ...
    prompt: `
    Build a Next.js SaaS dashboard that allows users to upload Excel files or connect to APIs, preview schema, configure widgets, and display display raw data table just 30 row below all 4 wieght. The interface must be fully interactive, scrollable, error-proof, and redirect users to a dashboard page after successful data connection.
...
📊 2. DASHBOARD - 4 DEFAULT WIDGETS (Renameable + Custom Graph Types)
Default Widget Setup:
const [widgets, setWidgets] = useState([
  { id: 'widget1', title: 'Widget 1', graphType: 'bar', columns: [], data: [] },
do the same widget2 = line','widget3'='pie', 'widget4'= 'scatter'
const [editingTitle, setEditingTitle] = useState<string | null>(null);
Available Graph Types:
const GRAPH_TYPES = [
  { value: 'bar', label: 'Bar Chart', requiredColumns: 1 },
  { value: 'stackedBar', label: '100% Stacked Bar', requiredColumns: 2 },
  { value: 'line', label: 'Line Chart', requiredColumns: 2 },
  { value: 'area', label: 'Area Chart', requiredColumns: 2 },
  { value: 'pie', label: 'Pie Chart', requiredColumns: 1 },
  { value: 'donut', label: 'Donut Chart', requiredColumns: 1 },
  { value: 'scatter', label: 'Scatter Plot', requiredColumns: 2 },
  { value: 'timeseries', label: 'Time Series Plot', requiredColumns: 2 },
  { value: 'boxplot', label: 'Box Plot', requiredColumns: 2 },
  { value: 'bubble', label: 'Bubble Chart', requiredColumns: 3 }
];
🎨 COLORFUL GRAPHS REQUIREMENT:
const COLORS = [random color];
// Apply colors to all chart components:
🕒 NEW CHART TYPES IMPLEMENTATION:
Time Series Plot (Date format: "2025-07-29T09:42:07.972Z" - ISO 8601):
case 'timeseries':
...
case 'timeseries':
  return rawData
    ...
    .map(row => ({
      date: new Date(row[columns[0]]).getTime(),
      value: Number(row[columns[1]]) || 0,
      originalDate: row[columns[0]]
    }))
    .sort((a, b) => a.date - b.date);
📈 STATISTICS SUMMARY REQUIREMENT:
Add below each graph:
const calculateStatistics = (data, graphType, columns) => {
  if (!data || data.length === 0) return null;
  const values = data.map(item => item.value || item.y || 0).filter(val => !isNaN(val));
  if (values.length === 0) return null;
  const sum = values.reduce((a, b) => a + b, 0);
  also do avg, max, min 
  return { total: sum, average: avg.toFixed(2), maximum: max, minimum: min, count: data.length };
};
// Display stats below each chart:
{stats && (
  <div className="mt-4 p-3 bg-gray-50 rounded-lg">
    <h4 className="font-semibold text-sm mb-2 text-gray-700">Statistics Summary</h4>
    <div className="grid grid-cols-3 gap-2 text-xs">
      <div><span className="font-medium">Total:</span> {stats.total}</div>
do the same on Average, Count, Max and Min

Widget Configuration Panel:
function WidgetConfig({ widget, onUpdate, schema }) {
  const graphType = GRAPH_TYPES.find(g => g.value === widget.graphType);
  return (
    <div className="bg-white p-4 rounded-lg border">
      ...
      <div className="flex items-center justify-between mb-4">
        {editingTitle === widget.id ? (
          <input
            ...
            className="text-lg font-semibold border-b-2 border-blue-500 bg-transparent"
            autoFocus
          />
        ) : (
          <h3
      ...
      <div className="mb-4">
        ...
        <select
          ...
          onChange={e => onUpdate(widget.id, 'graphType', e.target.value)}
        ...
        {widget.graphType === 'timeseries' && (
          <p className="text-xs text-gray-500 mt-1">
            📅 Date format: "2025-07-29T09:42:07.972Z" (ISO 8601)
          </p>
        )}
      </div>
      ...
      <div className="mb-4">
        <label className="block text-sm font-medium mb-2">
          Data Columns ({graphType?.requiredColumns} required)
        </label>
        {Array.from({ length: graphType?.requiredColumns || 1 }).map((_, index) => (
          <select
            key={index}
            value={widget.columns[index] || ''}
            onChange={e => {
              const newColumns = [...widget.columns];
              newColumns[index] = e.target.value;
              onUpdate(widget.id, 'columns', newColumns);
            }}
      ...
      <div className="min-h-[300px] border rounded p-4">
        {widget.columns.length >= (graphType?.requiredColumns || 1) ? (
          <RenderGraph widget={widget} data={dashboardData} />
        ) : (
          <div className="flex items-center justify-center h-full text-gray-500">
            Select columns to display {graphType?.label}
          </div>
        )}
      </div>
    </div>
...
function RenderGraph({ widget, data }) {
  const processedData = processDataForGraph(data, widget.graphType, widget.columns);
...
function processDataForGraph(rawData, graphType, columns) {
  // Process raw data based on graph type and selected columns
  switch (graphType) {
    case 'bar':
    case 'pie':
    case 'donut':
    ...
    case 'stackedBar':
      // Group by first column, stack by second column
      return rawData.reduce((acc, row) => {
        const key = row[columns[0]];
        const stackKey = row[columns[1]];
        const existing = acc.find(item => item.name === key);
        if (existing) {
          existing[stackKey] = (existing[stackKey] || 0) + 1;
        } else {
          acc.push({ name: key, [stackKey]: 1 });
        }
        return acc;
      }, []);
    case 'scatter':
    ...
    case 'area':
      return rawData.map((row, index) => ({
        name: row[columns[0]] || \`Point \${index}\`,
        value: Number(row[columns[1]]) || 0
      }));
    case 'bubble':
      return rawData.map(row => ({
        ...
        z: Number(row[columns[2]]),
      }));
    case 'boxplot':
      return columns.map(col => ({
        name: col,
        values: rawData.map(row => Number(row[col])).filter(v => !isNaN(v))
      }));

    // and etc.
    default:
      return [];
📐 3. LAYOUT - 4 Widget Grid
\`\`\`tsx
...
SAFE JSX RENDERING RULES
✅ Use Object.entries() and .map() for dynamic data
✅ Use String(value) and value ?? 'N/A' to safely render values
✅ Never render raw objects: ❌ {data}
✅ Always include key in loops
✅ Use Array.isArray(val) ? val.join(', ') : String(val) for arrays
FINAL OUTCOMES
✅ Fully working "Connect Data" modal with Excel and API support
✅ Scrollable + sticky connect button
✅ Dashboard with 4 configurable widgets
✅ 10 smart graph types with vibrant colors
✅ Statistics summary below each graph
✅ Time series with proper date formatting
✅ Safe rendering logic throughout
✅ Responsive, scrollable, resizable layout
✅ No DOM errors (e.g., loading prop bug)
✅ make sure that non Chart type not implemented.
    `},
  {
    emoji: "🛰️",
